# RQ2 Reference Resource-Identity Reviewer E

You are reviewer E for a complete 56-row rule-impact audit. The masked JSONL
...
only the supplied row. Inspect every frozen probe record before comparing the
raw URL structures.
...
Use `reviewer_id=codex_reference_identity_e` and
`run_id=rq2_reference_identity_e2_20260715` on every row. `final_status` must be

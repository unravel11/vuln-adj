...
| C04B | Method | Two doctoral students are the intended reviewers under a trained-analyst framing, and the calibration-1 distribution gate is prepared but blocked | E07F | Role-design and current operational-state claim only | Do not claim practitioner expertise, verified independence, completed recruitment, distribution, or human evidence | `SUPPORTED` |
| C04B | Method | The formal set contains a fixed 34-case shared-no-manual audit and a pre-registered 0.10 simple-only manual-loss gate | E07D, E07E | Label-free identifiability and falsification-design claim | Do not call the 8.43% zero-event bound a population miss-rate bound or safety result | `SUPPORTED` |

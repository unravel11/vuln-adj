...
| Scientific | V3.1 action-first/reason-second validity diagnostics | RQ3 | Prepare-only calibration-1 20, calibration-2 reserve 20, and formal 120 packets validate; stage locks empty; human labels 0 | Authors and two reviewers | Yes |
| Scientific | Frozen return validators and evaluator | Outcome-independent RQ3 analysis | Implemented, hash-bound, unit-tested, and exercised end to end with temporary synthetic labels; this is mechanical readiness only | Authors | No longer a blocker |
| Scientific | Shared no-manual-route falsification audit | Branch P manual-loss/coverage gate | Fixed 34-case derived audit exists; no human labels; its 8.43% zero-event bound would be sample-conditional, not a population bound | Authors and two reviewers | Yes for Branch P |
| Scientific | Analyst-bounded three-strategy differentiation or boundary | Central RQ3 framing | RQ1/RQ2 deterministic census exists; no analyst-backed alignment, reliability, coverage, shared-miss, or boundary result | Authors | Yes |
...
| Manuscript | JSS manuscript source | Complete paper | Routing-centric title/thesis, deterministic RQ1/RQ2, analyst-bounded RQ3, exactly three contribution ceilings, and Branch P/B contract remain at S2. Markdown and editable `elsarticle` zero drafts cover Introduction, Related Work, corpus, deterministic RQ1/RQ2, V3.1, Discussion branches, and threats; the prose is not author approved and RQ3 remains a placeholder | Authors | Yes until human results and author revision |
| Manuscript | Closest-work synthesis in the manuscript | Novelty/positioning | Zero draft, 24-paper audit, 17-key checked BibTeX source, and citation/evidence map include the closest intersection, public resources, same-task baselines, and overlap risks. The Bayesian CVSS item remains abstract/metadata-only and is used at that ceiling | Authors | No for zero-draft integrity; recheck final result-dependent citations |
...
| Format/artifact | Current JSS guide, template, declarations, and artifact checks | Venue compliance | Official Guide checked 2026-08-25; supporting Elsevier/CTAN sources checked 2026-08-26. Flat `elsarticle` 3.5 source, deterministic tables, no-human manifest, and temporary 22-page build/visual QA exist. Author-anonymization mode, final declarations/PIDs, result-bearing source, and final artifact remain unresolved | Authors | Yes at S6/S7 |
...
5. Freeze pre-adjudication results and apply reviewer-specific reliability,
   exact paired-direction, 34-case shared-miss, coverage, and
   `delta_manual=0.10` manual-loss gates. Preserve abstain, uncertain,
   disagreement, and failed fields.
6. Require at least 29 human conflict actions per reviewer and both reviewers'
   complete gate passes for Branch P. Fewer than 25 for either reviewer makes
   conflict recall interval-only.
